...
//////////////////////////////////////////////////////////////////////////
// Touch controls support (Android / touch-screen ports).
//////////////////////////////////////////////////////////////////////////

//! Callback interface implemented by the game-side touch overlay
//! (virtual joystick / buttons / edit mode). All coordinates are in
//! framebuffer pixels with (0,0) at the top-left corner.
struct ITouchOverlaySink
{
	//! A finger touched the screen. Return true if the event was consumed
	//! (overlay button, joystick or edit gesture grabbed this finger).
	virtual bool OnTouchDown( int idFinger, float fX, float fY ) = 0;
	//! A finger moved. Return true if this finger belongs to the overlay.
	virtual bool OnTouchMove( int idFinger, float fX, float fY, float fDX, float fDY ) = 0;
	//! A finger was lifted. Return true if this finger belonged to the overlay.
	virtual bool OnTouchUp( int idFinger, float fX, float fY ) = 0;
	//! Called once per frame before rendering; synthesize held-key events here.
	virtual void OnTouchUpdate() = 0;
	//! Called at the end of the game frame to draw the overlay.
	virtual void OnTouchRender() = 0;
	//! True while the layout editor is active (input module disables look drag).
	virtual bool IsTouchEditMode() const = 0;
	//! Master switch queried by the input device (touch_enabled cvar).
	virtual bool IsTouchEnabled() const = 0;
	//! A finger tapped the screen while the overlay is disabled. Used for the
	//! small "TOUCH OFF" recovery button so touch can be re-enabled from UI.
	//! Return true if the tap was consumed (no mouse emulation for it).
	virtual bool OnDisabledTap( float fX, float fY ) = 0;
	//! True while engine UI owns the screen (menu / pause): unclaimed taps
	//! drive the UI cursor (absolute) instead of camera look.
...
struct IInput
{
	...
	//! Register the touch overlay controller (implemented by the game module).
	//! The input device routes raw finger events to this sink first; anything
	//! the sink claims (buttons, joystick, edit-mode gestures) is not turned
	//! into mouse-look movement.
	virtual void SetTouchOverlaySink( struct ITouchOverlaySink *pSink ) = 0;
	virtual struct ITouchOverlaySink *GetTouchOverlaySink() = 0;

	//! Inject a virtual key press/release as if it came from a physical device.
	//! Updates key state tables and broadcasts a normal SInputEvent, so action
	//! maps, scripts and UI react exactly like to hardware input.
	virtual void PostVirtualKeyEvent( int xkey, bool bKeyDown ) = 0;
	virtual void PostVirtualMouseMove( float dx, float dy ) = 0;
};
